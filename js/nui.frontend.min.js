...
else{return false;}}};}());nui.popup=(function(){var popups={},default_settings={template:'t1',skin:'skin1',dialog_width:700,dialog_height:700,dialog_custom_css:false,mask_opacity:0.7,mask_custom_css:false,anim_fadeIn_speed:500,anim_fadeOut_speed:400,cookie_name:false,cookie_expires_days:14,cookie_disabled:false,ga_tracking_page:false,close_title:"Close",onClose:function(){}};function display(id){var docH,offset_pos,popup,settings;if(popups.hasOwnProperty(id)){stopMoving=true;settings=popups[id].settings;popup=popups[id];docH=$(document).height();popup.content.show();popup.$dialog.append(popup.content).append(popup.$close).css('marginLeft',-(settings.dialog_width/2)).width(settings.dialog_width).hide();popup.$mask.height(docH).hide();$('body').append(popup.$mask).append(popup.$dialog);popup.$mask.add(popup.$close).bind('click',function(){close(id);});popup.$dialog.fadeTo(0,0.001,function(){var d_offset=popup.$dialog.height()/2+50;if($(this).height()>docH){$(this).css('marginTop',-d_offset);}
...
else{cpopup.settings[key]=val;}}},load:function(id,url,sel,onClose){var $nonce=$('<div class="nonce">');if(sel){url=url+' '+sel;}
$nonce.load(url,function(response,status){popups[id].content=$nonce;display(id,onClose);});},frag:function(id,$obj,onClose){var c_popup=popups[id];if(typeof $obj==='string'){$obj=$($obj);}
